'''Simply inject the normal canvas into the selector, use the selector from the sprite creator'''
...
class SpriteTileSelectorLabel(QLabel):
...
class SpriteTileSelectorMain(QWidget):
    ...
    def display_active_tileset(self, name):
        ...
        for index, tile_pixmap in enumerate(self.model.tilesets[name]):
            ...
            tile = SpriteTileSelectorLabel(tile_name=name, pixmap=tile_pixmap, tile_index=index, selector= self)
